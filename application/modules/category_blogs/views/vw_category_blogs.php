    ...
    <section class="section">
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <a href="<?= base_url('Category_blogs/tambah') ?>" class="btn btn-primary mb-2">Tambah</a>
...
<script>
    $(document).ready(function() {
        $('#table-data').DataTable({
            ajax: '<?= base_url('category_blogs/load') ?>',
            processing: true,
            // scrollX: true,
            // serverSide: true,
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return `<img src="<?= base_url('assets/uploads/') ?>category_images/${data}" alt="" width="100px">`
                    }
                },
                {
                    data: 'is_active',
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return `<div>
                                ...
                                <a href="<?= base_url() ?>category_blogs/edit/${data}" class="btn btn-sm btn-info">Edit</a>
                                </div>`;
                    }
                }
            ]
    ...
    function hapus(uuid) {
        ...
        cuteAlert({
            ...
            if (e == "confirm") {
                $.ajax({
                    url: '<?= base_url() ?>/category_blogs/hapus/',
                    type: 'POST',
                    dataType: 'JSON',
                    data: {
                        uuid
    ...
    function published(uuid) {
        $.ajax({
            url: '<?= base_url() ?>category_blogs/updated_published/' + uuid,
            method: 'POST',
            dataType: 'JSON',
            success: function(results) {
                if (results.code != 200) {
    ...
    function drafted(uuid) {
        $.ajax({
            url: '<?= base_url() ?>category_blogs/updated_drafted/' + uuid,
            method: 'POST',
            dataType: 'JSON',
            success: function(results) {
                if (results.code != 200) {
